                    ...
                    <?php 
                          ...
                          else
                          {
                            ...
                            while($row=mysqli_fetch_assoc($sql))
                            {
                              ...
                              <td><a href='dasbor_perusahaan.php?aksi=delete&id=".$row['nipernyo']."'><button type='button' class='btn btn-default' aria-label='Left Align'>
...
<!-- Fungsi Menghapus -->
      ...
      if(isset($_GET['aksi']) == 'delete'){
        $id = $_GET['id'];
        $cek = mysqli_query($koneksi, "SELECT * FROM perusahaan WHERE nipernyo='$id'");
        if(mysqli_num_rows($cek) == 0){
          echo "<script>alert('Data Tidak Ditemukan');history.go(-1);</script>";
        }else{
          $delete = mysqli_query($koneksi, "DELETE FROM perusahaan WHERE nipernyo='$id'");
          if($delete){

                    ...
                    <?php 
                          ...
                          else
                          {
                            ...
                            while($row=mysqli_fetch_assoc($sql))
                            {
                              ...
                              <td>".$row['websitenyo']."</td>
                              <td>".$row['katorahasio_sekolah']."</td>
                              <td><a href='dasbor_sekolah.php?aksi=delete&id=".$row['npsnyo']."'><button type='button' class='btn btn-default' aria-label='Left Align'>
...
<script src="../js/jquery.dataTables.js"></script>
 ...
 <!-- Fungsi Menghapus -->
      ...
      if(isset($_GET['aksi']) == 'delete'){
        $id = $_GET['id'];
        $cek = mysqli_query($koneksi, "SELECT * FROM sekolah WHERE npsnyo='$id'");
        if(mysqli_num_rows($cek) == 0){
          echo "<script>alert('Data Tidak Ditemukan');history.go(-1);</script>";
        }else{
          $delete = mysqli_query($koneksi, "DELETE FROM sekolah WHERE npsnyo='$id'");
          if($delete){

...
if(isset($_POST['submit']))
{
  ...
  $cek = mysqli_query($koneksi, "SELECT * FROM sekolah WHERE npsnyo='$npsnyo'");
  if(!$cek)
  {
    echo "Gagal";
  }
  $data = mysqli_fetch_assoc($cek);
  $passwordnya = $data['katorahasio_sekolah'];
  $nisnnya = $data['npsnyo'];
  ...
  {
    // echo "Berhasil Login";
    $_SESSION['sekolah'] = $data['npsnyo'];
    echo "<script>alert('Login Berhasil');history.go(-1);</script>";
    header("Location: sekolah_detail.php");
  }else
  {
...
<nav class="navbar navbar-default navbar-fixed-top">
  <div class="container">
    ...
    <div class="navbar-header">
        <div class="row">
          <div class="info">    
             <a href="#"><button type="button" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-info-sign" aria-hidden="true"></span> Petunjuk Teknis</button></a>
          </div>
        </div>
    </div><!-- /.navbar-collapse -->
  </div><!-- /.container-fluid -->
...
<div class="kotak-bawah">
  ...
  <div class="col-md-5 col-md-offset-3">
    <h4>Login Sekolah</h4>
    <p>Login ke halaman dashboard sekolah dengan menggunakan NPSN dan Password yang telah didapatkan pada saat mendaftar </p>


         <form class="form-horizontal" method="post">
          <div class="form-group">
            <label for="inputEmail3" class="col-sm-2 control-label">NPSN</label>
              <div class="col-sm-10"><input type="text" class="form-control" id="inputEmail3" placeholder="Nomor Pokok Sekolah Nasional" name="npsnyo"></div>
          </div>
        <div class="form-group">
          <label for="inputPassword3" class="col-sm-2 control-label">Password</label>
          <div class="col-sm-10"><input type="password" class="form-control" id="inputPassword3" placeholder="Password" name="password"></div>
        </div>
